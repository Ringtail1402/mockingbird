...
/**
 * Base class that represents a row from the 'budgets' table.
 *
 *
 *
...
abstract class BaseBudget extends BaseObject implements Persistent
{
     ...
     * Peer class name
     */
    const PEER = 'Mockingbird\\Model\\BudgetPeer';
    ...
    public function setUserId($v)
    {
        ...
        if ($this->user_id !== $v) {
            $this->user_id = $v;
            $this->modifiedColumns[] = BudgetPeer::USER_ID;
        }
    ...
    public function setYear($v)
    {
        ...
        if ($this->year !== $v) {
            $this->year = $v;
            $this->modifiedColumns[] = BudgetPeer::YEAR;
        }
    ...
    public function setMonth($v)
    {
        ...
        if ($this->month !== $v) {
            $this->month = $v;
            $this->modifiedColumns[] = BudgetPeer::MONTH;
        }
    ...
    public function setCurrencyId($v)
    {
        ...
        if ($this->currency_id !== $v) {
            $this->currency_id = $v;
            $this->modifiedColumns[] = BudgetPeer::CURRENCY_ID;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false)
    {
        try {
            ...
            return $startcol + 5; // 5 = BudgetPeer::NUM_HYDRATE_COLUMNS.
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(BudgetPeer::ID)) {
            $modifiedColumns[':p' . $index++]  = '`id`';
        }
        if ($this->isColumnModified(BudgetPeer::USER_ID)) {
            $modifiedColumns[':p' . $index++]  = '`user_id`';
        }
        if ($this->isColumnModified(BudgetPeer::YEAR)) {
            $modifiedColumns[':p' . $index++]  = '`year`';
        }
        if ($this->isColumnModified(BudgetPeer::MONTH)) {
            $modifiedColumns[':p' . $index++]  = '`month`';
        }
        if ($this->isColumnModified(BudgetPeer::CURRENCY_ID)) {
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(BudgetPeer::USER_ID)) $criteria->add(BudgetPeer::USER_ID, $this->user_id);
        if ($this->isColumnModified(BudgetPeer::YEAR)) $criteria->add(BudgetPeer::YEAR, $this->year);
        if ($this->isColumnModified(BudgetPeer::MONTH)) $criteria->add(BudgetPeer::MONTH, $this->month);
        if ($this->isColumnModified(BudgetPeer::CURRENCY_ID)) $criteria->add(BudgetPeer::CURRENCY_ID, $this->currency_id);
    ...
    public function getCurrency(PropelPDO $con = null, $doQuery = true)
    {
        if ($this->aCurrency === null && ($this->currency_id !== null) && $doQuery) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aCurrency->addBudgets($this);
             */
        }
    ...
    public function clearEntrys()
    {
        ...
        $this->collEntrysPartial = null;

        return $this;
    }

    /**
     * reset is the collEntrys collection loaded partially
     *
     * @return void
     */
    public function resetPartialEntrys($v = true)
    {
        $this->collEntrysPartial = $v;
    }
    ...
    public function initEntrys($overrideExisting = true)
    {
        ...
        $this->collEntrys->setModel('BudgetEntry');
    }
    ...
    {
        $partial = $this->collEntrysPartial && !$this->isNew();
        if (null === $this->collEntrys || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collEntrys) {
                // return empty collection
                $this->initEntrys();
            } else {
                $collEntrys = BudgetEntryQuery::create(null, $criteria)
                    ->filterByBudget($this)
                    ->find($con);
                if (null !== $criteria) {
    ...
    {
        $entrysToDelete = $this->getEntrys(new Criteria(), $con)->diff($entrys);


        $this->entrysScheduledForDeletion = $entrysToDelete;

        foreach ($entrysToDelete as $entryRemoved) {
            $entryRemoved->setBudget(null);
        }
     ...
     * @return Budget The current object (for fluent API support)
     */
    public function addEntry(BudgetEntry $l)
    {
        if ($this->collEntrys === null) {
            $this->initEntrys();
            $this->collEntrysPartial = true;
    ...
    /**
     * @param  Entry $entry The entry object to remove.
     * @return Budget The current object (for fluent API support)
     */
    public function removeEntry($entry)
    {
        if ($this->getEntrys()->contains($entry)) {
            $this->collEntrys->remove($this->collEntrys->search($entry));
            if (null === $this->entrysScheduledForDeletion) {
                $this->entrysScheduledForDeletion = clone $this->collEntrys;
                $this->entrysScheduledForDeletion->clear();
            }
            $this->entrysScheduledForDeletion[]= clone $entry;
     ...
     * @return PropelObjectCollection|BudgetEntry[] List of BudgetEntry objects
     */
    public function getEntrysJoinCategory($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = BudgetEntryQuery::create(null, $criteria);
